...
Ensure your `backend/.env` file matches the credentials in `docker/docker-compose.yml`:
...
# Kill the process or change the port in docker/docker-compose.yml
```
...
# Limit container memory in docker/docker-compose.yml
services:
  ollama:
    deploy:
      resources:
...
docker compose -f docker/docker-compose.yml port backend 8000
```
